...
def build_model(learning_rate: float):
    ...
    # linear regression model is a single node in a single layer
    model.add(keras.layers.Dense(units=1, input_shape=(1, )))

    # compile the model topography into code that tf can execute.
    # configure training to minimize the model's MSE
    model.compile(
...
def train_model(model: keras.Model, feature, label, epochs: int,
                batch_size: int):
    ...
    trained_params = model.get_weights()
    weight = trained_params[0]
    bias = trained_params[1]
    ...
    return weight, bias, trained_epochs, rmse
...
# Import the dataset.
training_df: pd.DataFrame = pd.read_csv(
    filepath_or_buffer=
...
def predict_house_values(n, feature, label):
    ...
    for i in range(n):
        print("%5.0f %6.0f %15.0f" %
              (training_df[feature][10000 + i], training_df[label][10000 + i],
